...
## 状態ファイル（過渡期の dual-write）

launcher は従来の `active-route`・`time-sync-required` ファイル（正本）に加え、
schema 1 の `runtime-state.json` を発行する。読み手の切替は backlog
BL-01（`TASKS.md`）で行い、それまで watcher は従来ファイルを読む。
protocol の詳細と fail-closed 規則は `sushida_os/runtime/runtime_state.py` と
