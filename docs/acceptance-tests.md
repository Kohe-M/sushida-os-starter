...
| R5 | D03, D12（UEFI smoke） | QEMU | `c9dd1ad` | `aebbd14b36673ac2360a3590b98cb904746b24bdea748d1fd0f9ec95216bf745` | 同上、UEFI smoke（OVMF + TCG） | screenshot 完全性判定が不成立（OVMF+TCG では post-GOP scanout が黒のままになり得る既知制約。serial では graphical.target・全 service 起動を確認）。R6 の KVM 再実施で解消 | FAIL （環境制約: OVMF+TCG。KVM または実機で要再実施） | 2026-07-21 | agent (Claude) | R6 の再実施で run dir を上書き（serial 相当証跡は R6 に含む） |
| R6 | D03, D12（UEFI smoke 再実施） | QEMU | `c9dd1ad` | `aebbd14b36673ac2360a3590b98cb904746b24bdea748d1fd0f9ec95216bf745` | QEMU 10.x KVM（builder コンテナ + `/dev/kvm`）、UEFI smoke 観測 300s | `--qemu-smoke` UEFI（OVMF）。AUTOMATED 8 判定すべて成立: ISO SHA 一致・観測完走・screenshot PNG 取得・screenshot 完全性・login prompt 不在・backend/watcher エラー不在・kiosk/graphical 到達・config FS + Wi-Fi service 起動 | PASS | 2026-07-22 | agent (Claude) | `build/qemu/uefi-offline/{screenshot.png,serial.plain.log,result.env,smoke-report.txt}` |
| R7 | D01, D02（実機フィードバック版） | build | `38fccb6` | `dc82bca1c031cd1403619e867af295855e33b6ec971437ce5f68902760963d98` | privileged builder（podman）+ keep-id verify | FB-01〜FB-04 反映イメージの `make container-iso` + `verify-iso.sh`（exact 33件 byte/mode/owner・partition・metadata 相互照合） | PASS | 2026-07-22 | agent (Claude) | `artifacts/{sushida-os-amd64.iso,SHA256SUMS,build-info.json,package-manifest.txt}` |
| R8 | D03, D12（FB 版 QEMU 回帰） | QEMU | `38fccb6` | `dc82bca1c031cd1403619e867af295855e33b6ec971437ce5f68902760963d98` | QEMU 10.x KVM（builder コンテナ + `/dev/kvm`）、各 240s | BIOS boot-test（production bootloader→kiosk 到達）+ BIOS/UEFI smoke 各 8 判定すべて PASS。screenshot 目視で 200% ズーム実効（旧 1x 比で文字 2 倍・全画面維持）を確認。旧 `--force-device-scale-factor` 案は 1/4 描画バグを QEMU で検出し廃案（`907abf1`） | PASS | 2026-07-22 | agent (Claude) | `build/qemu/{bios,uefi}-offline/{screenshot.png,serial.plain.log,result.env}` |
