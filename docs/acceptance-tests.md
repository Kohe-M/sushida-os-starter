...
| R5 | D03, D12（UEFI smoke） | QEMU | `c9dd1ad` | `aebbd14b36673ac2360a3590b98cb904746b24bdea748d1fd0f9ec95216bf745` | QEMU 10.x KVM（builder コンテナ + `/dev/kvm`）、UEFI smoke 観測 300s | `--qemu-smoke` UEFI（OVMF）。AUTOMATED 8 判定すべて成立: ISO SHA 一致・観測完走・screenshot PNG 取得・screenshot 完全性・login prompt 不在・backend/watcher エラー不在・kiosk/graphical 到達・config FS + Wi-Fi service 起動 | PASS | 2026-07-22 | agent (Claude) | `build/qemu/uefi-offline/{screenshot.png,serial.plain.log,result.env,smoke-report.txt}` |
| R5a | 同上（旧記録） | QEMU | `c9dd1ad` | 同上 | UEFI smoke（OVMF + TCG、KVM なし） | screenshot 完全性判定が不成立（OVMF+TCG では post-GOP scanout が黒のままになり得る既知制約。serial では graphical.target・全 service 起動を確認）。KVM 有効化後の R5 再実施で解消 | FAIL （環境制約: OVMF+TCG） | 2026-07-21 | agent (Claude) | R5 の再実施で上書き（serial 証跡は R5 に含む） |
