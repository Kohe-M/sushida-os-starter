...
#
# The verification stages are plain functions so tests can source this file
# and exercise each stage against fixture artifacts; executing the script
# always runs every stage in order (there is no environment bypass).
set -euo pipefail
...
verify_checksums() {
    local checksum_line_count expected_name
...
verify_metadata() {
    ...
    local current_commit git_status metadata_commit computed_sha metadata_sha
    ...
    local package version metadata_version
    ...
    # The artifact set must have been built against this exact manifest, and
    # the published package manifest must be the one recorded at build time.
    local contract_sha metadata_contract_sha manifest_sha metadata_manifest_sha
...
verify_iso_root() {
    ...
    local req_path req_pattern req_match inner_re
...
verify_partitions() {
    local system_area config_start config_label config_type
...
verify_squashfs() {
    ...
    # Mappings with current_verification "exact" are byte-compared against
    # the tracked source tree, and their mode/owner/group inside the image
    # must match the contract declaration.  Presence alone is insufficient
    # for these files: a stale ISO built from an older worktree revision is
    # rejected.
    local map_source map_image map_mode map_owner map_group
    local src inner inner_glob extracted entry symbolic
    while IFS= read -r -d '' map_source && \
        ...
        inner="${map_image#/}"
        # unsquashfs matches extract paths as globs; escape glob characters
        # and backslashes so the contract path is taken literally.
...
# Executing the script always runs the full verification; sourcing it (for
# stage-level tests) only defines the functions.
