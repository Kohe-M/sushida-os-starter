...
#
# The run is organized as stage functions invoked in a fixed order by
# main(); sourcing this file only defines them.  Behavior, messages, and
# the assembled QEMU argument order are part of the tested contract.
set -euo pipefail
...
# Print and exit before touching files or directories.
print_dry_run() {
...
install_cleanup_trap() {
    ...
    # Register the trap before any extraction so that a failure during
    # xorriso or symlink resolution still removes the temporary directory.
}

# ── Extract kernel and initrd from ISO for direct boot ──────────────────
extract_smoke_kernel() {
    [ "$QEMU_SMOKE" = true ] || return 0
...
# Without a bounded duration the run is interactive: wait for QEMU itself.
wait_unbounded() {
    [ "$DURATION" -eq 0 ] || return 0
    local status
    set +e
    wait "$QEMU_PID"
    status=$?
...
wait_boot_evidence() {
    [ "$QEMU_BOOT_TEST" = true ] || return 0
    local kiosk_ready=false _boot
...
run_powerdown() {
    ...
    local kiosk_ready=false _boot _shutdown
    for _boot in $(seq 1 "$DURATION"); do
...
observe_and_capture() {
    ...
    if [ "$QEMU_SMOKE" = true ]; then
        ...
        local screenshot_ready=false _capture
...
# Executing the script always runs the full flow; sourcing it (for tests)
# only defines the stage functions.
